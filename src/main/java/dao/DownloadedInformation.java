 ...
 * Shows which data has been downloaded by other data suppliers. Based on
...
public class DownloadedInformation {
     ...
     *            Download date to
     * @param fdpContent
     *            Content type (FUND, DOC, REG)
     * @param params
     *            Optional download parameters for filtering
     * @return List of downloaded information entries
     */
    public List<DownloadedInformationEntry> getDownloadedInformationEntries(LocalDate dateFrom, LocalDate dateTo,
        ...
        if (applicationSettings.isFileSystem()) {
            ...
            File[] potentialFiles = backupDir.listFiles((dir, name) -> name.contains("DOWNLOAD_OWN_DATA_DOWNLOADED.xml")
                    || name.contains("DOWNLOADED_INFORMATION.xml"));

            LOG.debug("Potential downloaded information files: "
    ...
    private DownloadedInformationEntry parseDownloadedInformationEntry(Element element) {
        ...
        if (!downloadDateTimeStr.isEmpty()) {
            try {
                LocalDateTime downloadDateTime = LocalDateTime.parse(downloadDateTimeStr,
        ...
        if (fundOrShareClassNodes.getLength() > 0) {
            ...
            // OeNB_Identnr
            String oenbId = getElementText(fundElement, "OeNB_Identnr");
            if (!oenbId.isEmpty()) {
                entry.setOenbIdentnr(oenbId);
            }
