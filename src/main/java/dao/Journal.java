...
/**
 * DAO for retrieving and parsing Journal data from OeKB FDP. Based on
...
public class Journal {
     ...
     *
     * @param timeFrom
     *            Start date/time filter (optional)
     * @param timeTo
     *            End date/time filter (optional)
     * @param action
     *            Filter by action type: UL (Upload) or DL (Download) (optional)
     * @param type
     *            Filter by journal type (optional)
     * @param excludeEmptyDownloads
     *            Exclude empty data downloads (default: true)
     * @return List of journal entries
     */
    public List<JournalEntry> getJournalEntries(LocalDateTime timeFrom, LocalDateTime timeTo, String action,
            String type, boolean excludeEmptyDownloads) {
        String xmlString;
        ...
        if (applicationSettings.isFileSystem()) {
            ...
            File[] potentialFiles = backupDir.listFiles((dir, name) -> name.contains("DOWNLOAD_JOURNAL.xml"));

            LOG.debug(
                    "Potential journal files: " + (potentialFiles != null ? Arrays.toString(potentialFiles) : "none"));
            ...
            xmlString = new OeKBHTTP().downloadJournal(timeFrom, timeTo, action, type, null, null,
                    excludeEmptyDownloads);
        }
