...
/**
 * Model class representing downloaded information from OeKB FDP. Shows which
 * data has been downloaded by other data suppliers. Based on
 * FundsXML_DownloadedInformation_1.0.0.xsd
 */
public class DownloadedInformationEntry {

    public enum ContentType {
        FUND("Fund Data"), DOC("Documents"), REG("Regulatory Reporting");
    ...
    public List<String> getDataSuppliers() {
        return new ArrayList<>(dataSuppliers);
    }

    public void setDataSuppliers(List<String> dataSuppliers) {
        this.dataSuppliers = dataSuppliers == null ? new ArrayList<>() : new ArrayList<>(dataSuppliers);
    }
    ...
    public String getOenbIdentnr() {
        return oenbIdentnr;
    }

    public void setOenbIdentnr(String oenbIdentnr) {
        this.oenbIdentnr = oenbIdentnr;
    }
    ...
    public void setIsins(List<String> isins) {
        this.isins = isins == null ? new ArrayList<>() : new ArrayList<>(isins);
    }
    ...
    public String getIdentifierSummary() {
        ...
        if (oenbIdentnr != null && !oenbIdentnr.isEmpty()) {
            if (sb.length() > 0) {
                sb.append(", ");
            }
            sb.append("OeNB: ").append(oenbIdentnr);
        }
        if (!isins.isEmpty()) {
            if (sb.length() > 0) {
                sb.append(", ");
    ...
    public String toString() {
        return "DownloadedInformationEntry{" + "contentType=" + contentType + ", contentDate=" + contentDate
                + ", downloadDateTime=" + downloadDateTime + ", dataSuppliers=" + dataSuppliers.size() + ", lei='" + lei
                + '\'' + ", isins=" + isins.size() + ", accessRuleId='" + accessRuleId + '\'' + ", isOwner=" + isOwner
                + '}';
    }
}
