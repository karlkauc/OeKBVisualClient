...
/**
 * Model class representing upload reply/validation results from OeKB FDP. Based
 * on FundsXML_Reply_3.0.1.xsd
 */
public class UploadReply {

    public enum OverallStatus {
        OK("OK - All data processed successfully"), OK_INFOS(
                "OK with Infos - Data processed, please review notes"), ERROR(
                        "ERROR - Some data could not be processed"), FILE_NOT_PROCESSED(
                                "FILE NOT PROCESSED - Global error occurred");
    ...
    public static class AccessRuleStatus {
        ...
        public List<ElementStatus> getElementStatuses() {
            return new ArrayList<>(elementStatuses);
        }

        public void setElementStatuses(List<ElementStatus> elementStatuses) {
            this.elementStatuses = elementStatuses == null ? new ArrayList<>() : new ArrayList<>(elementStatuses);
        }
    ...
    public List<StatusInfo> getStatusInfos() {
        return new ArrayList<>(statusInfos);
    }

    public void setStatusInfos(List<StatusInfo> statusInfos) {
        this.statusInfos = statusInfos == null ? new ArrayList<>() : new ArrayList<>(statusInfos);
    }
    ...
    public List<AccessRuleStatus> getAccessRuleStatuses() {
        return new ArrayList<>(accessRuleStatuses);
    }

    public void setAccessRuleStatuses(List<AccessRuleStatus> accessRuleStatuses) {
        this.accessRuleStatuses = accessRuleStatuses == null ? new ArrayList<>() : new ArrayList<>(accessRuleStatuses);
    }
    ...
    public String toString() {
        return "UploadReply{" + "dataSupplier='" + dataSupplierShort + '\'' + ", uniqueDocumentId='" + uniqueDocumentId
                + '\'' + ", overallStatus=" + overallStatus + ", errors=" + getErrorCount() + ", infos="
                + getInfoCount() + '}';
    }
}
