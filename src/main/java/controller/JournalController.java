...
public class JournalController implements Initializable {
    ...
    private void setupTable() {
        ...
        timestampCol.setCellFactory(column -> new TableCell<>() {
            @Override
        ...
        journalTable.getColumns().addAll(timestampCol, actionCol, typeCol, dataSupplierCol, userCol, uniqueIdCol,
                detailsCol);
    ...
    private void loadJournalEntries() {
        ...
        try {
            ...
            LOG.debug("Loaded {} journal entries", entries.size());
    ...
    void exportToExcel() {
        ...
        fileChooser.setInitialFileName(
                LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd_HH-mm-ss")) + "_journal.xlsx");
        ...
        if (file != null) {
            try {
                ...
                alert.setContentText(
                        "An error occurred while writing the journal data to the selected file:\n" + e.getMessage());
                alert.showAndWait();
            }
        } else {
    ...
    void filterTable() {
        ...
        if (searchText.isEmpty()) {
            ...
            List<JournalEntry> matches = allEntries.stream().filter(entry -> matchesSearchCriteria(entry, searchText))
