...
/**
 * Controller for New Information page - displays notifications about newly
 * available data
 */
public class NewInformationController implements Initializable {
    ...
    private void setupComboBoxes() {
        ...
        contentTypeComboBox.setItems(FXCollections.observableArrayList("All", "FUND - Fund Data", "DOC - Documents",
    ...
    private void setupTable() {
        ...
        contentDateCol.setCellValueFactory(
                cellData -> new SimpleStringProperty(cellData.getValue().getFormattedContentDate()));
        ...
        dataSupplierCol.setCellValueFactory(
                cellData -> new SimpleStringProperty(cellData.getValue().getDataSuppliersString()));

        // Identifiers column
        TableColumn<NewInformationEntry, String> identifiersCol = new TableColumn<>("Identifiers");
        identifiersCol.setPrefWidth(200);
        identifiersCol
                .setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getIdentifierSummary()));
        ...
        detailsCol.setCellFactory(column -> new TableCell<>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
        ...
        profilesCol.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getProfilesString()));

        newInformationTable.getColumns().addAll(typeCol, contentDateCol, uploadDateCol, dataSupplierCol, identifiersCol,
                detailsCol, profilesCol);
        ...
        new Thread(() -> {
            try {
                ...
                LocalDateTime uploadFrom = uploadFromPicker.getValue() != null
                        ? LocalDateTime.of(uploadFromPicker.getValue(), LocalTime.MIN)
                        : null;
                LocalDateTime uploadTo = uploadToPicker.getValue() != null
                        ? LocalDateTime.of(uploadToPicker.getValue(), LocalTime.MAX)
                        : null;

                LOG.debug("Fetching new information entries: contentDate={}, uploadFrom={}, uploadTo={}", contentDate,
                        uploadFrom, uploadTo);

                List<NewInformationEntry> entries = newInformationDao.getNewInformationEntries(contentDate, uploadFrom,
                        uploadTo);
    ...
    private void filterEntries() {
        ...
        List<NewInformationEntry> filtered = allEntries.stream().filter(entry -> {
            // Filter by content type
            if (!"All".equals(selectedType)) {
                String typePrefix = selectedType.split(" ")[0]; // Extract FUND, DOC, or REG
                if (entry.getContentType() == null || !entry.getContentType().name().equals(typePrefix)) {
                    return false;
                }
            }
            ...
            if (!searchText.isEmpty()) {
                String entryText = String
                        .format("%s %s %s %s %s %s %s", entry.getContentType(), entry.getFormattedContentDate(),
                                entry.getFormattedUploadDateTime(), entry.getDataSuppliersString(),
                                entry.getIdentifierSummary(), entry.getDetails(), entry.getProfilesString())
