...
/**
 * Controller for the Access Rule Edit Dialog Provides comprehensive editing of
 * access rules with tabs for different sections
 */
public class AccessRuleEditDialog implements Initializable {
    ...
    private final ObservableList<String> dataSuppliers = FXCollections.observableArrayList();
    private final ObservableList<String> profiles = FXCollections.observableArrayList();
    private final ObservableList<String> leiList = FXCollections.observableArrayList();
    private final ObservableList<String> oenbIdList = FXCollections.observableArrayList();
    private final ObservableList<String> segmentIsinList = FXCollections.observableArrayList();
    private final ObservableList<String> shareclassIsinList = FXCollections.observableArrayList();
    private final ObservableList<String> documentTypes = FXCollections.observableArrayList();
    private final ObservableList<String> regulatoryReportings = FXCollections.observableArrayList();
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        newProfileCombo.setItems(FXCollections.observableArrayList("all", "PKG", "Vendor", "allOhneSegmente",
        ...
        SpinnerValueFactory<Integer> valueFactory = new SpinnerValueFactory.IntegerSpinnerValueFactory(0, 365, 0, 1);
        accessDelaySpinner.setValueFactory(valueFactory);
        accessDelaySpinner.setEditable(true);

        // Setup Document Types ComboBox
        newDocumentTypeCombo.setItems(FXCollections.observableArrayList("AIFMD", "AnnualReport", "AuditReport",
                "Factsheet", "KID", "Prospectus", "PRIIPS-KID"));

        // Setup Regulatory Reporting ComboBox
        newRegulatoryReportingCombo
                .setItems(FXCollections.observableArrayList("EMIR", "KIID", "EMT", "TPTSolvencyII", "PRIIPS"));
        ...
        // Setup TableViews
        leiColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue()));
        leiTableView.setItems(leiList);

        oenbIdColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue()));
        oenbIdTableView.setItems(oenbIdList);

        segmentIsinColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue()));
        segmentIsinTableView.setItems(segmentIsinList);

        shareclassIsinColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue()));
    ...
    private void loadAccessRuleData() {
        ...
        if (accessRule.getCostsByDataSupplier() != null) {
            costsByDataSupplierCheckBox.setSelected("true".equalsIgnoreCase(accessRule.getCostsByDataSupplier())
        ...
        if (accessRule.getOenbId() != null) {
            oenbIdList.setAll(accessRule.getOenbId());
        }
        if (accessRule.getIsinSegment() != null) {
            segmentIsinList.setAll(accessRule.getIsinSegment());
        }
        if (accessRule.getIsinShareclass() != null) {
            shareclassIsinList.setAll(accessRule.getIsinShareclass());
        }
    ...
    private void saveAccessRuleData() {
        ...
        accessRule.setOenbId(new ArrayList<>(oenbIdList));
        accessRule.setIsinSegment(new ArrayList<>(segmentIsinList));
        accessRule.setIsinShareclass(new ArrayList<>(shareclassIsinList));
        ...
        if (isInputValid()) {
            ...
            if (isNewRule) {
                if (isFileSystemMode) {
                    ...
                    confirmAlert.setContentText("OFFLINE MODE - No server upload will be performed.\n\n"
                    ...
                    confirmAlert.setContentText("This will upload a new access rule to server.\n\n" + "Rule ID: "
                            + accessRule.getId() + "\n\n" + "Do you want to continue?");
                }
            } else {
                if (isFileSystemMode) {
            ...
            new Thread(() -> {
                try {
                    LOG.info("Starting save workflow for rule: {} (New: {}, FileSystem Mode: {})", accessRule.getId(),
                            isNewRule, isFileSystemMode);
                    ...
                    if (!isNewRule) {
                        ...
                        if (isFileSystemMode) {
                            ...
                            if (deleteResult.startsWith("ERROR")) {
                                throw new IllegalStateException("Delete failed: " + deleteResult);
                            }
                        }
                    }
                    ...
                    if (isFileSystemMode) {
                        ...
                        if (importResult.startsWith("ERROR")) {
                            throw new IllegalStateException("Import failed: " + importResult);
                        }
                    }
                    ...
                    Platform.runLater(() -> {
                        ...
                        if (isNewRule) {
                            if (isFileSystemMode) {
                                showSuccess("Success (Offline Mode)", "New access rule created!\n\n"
                            ...
                            if (isFileSystemMode) {
                                ...
                                showSuccess("Success", "Access rule updated successfully!\n\n"
                    ...
                    Platform.runLater(() -> {
                        showUploadError("Save Failed", "Could not save access rule:\n\n" + e.getMessage());
    ...
    /**
     * Validate user input based on XSD mandatory fields XSD:
     * FundsXML_AccessRules_2.3.0.xsd
     *
     * Mandatory fields per XSD schema: - id attribute (use="required") -
     * ContentType element - DataSuppliers/DataSupplier (at least one) -
     * Profiles/Profile (at least one) - AccessObjects/AccessObject (at least one)
     */
    private boolean isInputValid() {
        StringBuilder errorMessage = new StringBuilder(768);

        // 1. Rule ID - MANDATORY (XSD line 310: attribute id use="required")
        if (ruleIdField.getText() == null || ruleIdField.getText().isBlank()) {
        ...
        // 3. DataSuppliers - MANDATORY (XSD line 47-60: at least one DataSupplier
        // required)
        ...
        // 5. AccessObjects - MANDATORY (XSD line 91-189: at least one AccessObject
        // required)
        // Each AccessObject must have either Fund (LEI or OeNB_ID), Segment (ISIN), or
        // ShareClass (ISIN)
        boolean hasAccessObject = !leiList.isEmpty() || !oenbIdList.isEmpty() || !segmentIsinList.isEmpty()
                || !shareclassIsinList.isEmpty();

        if (!hasAccessObject) {
            errorMessage.append("• At least one Access Object is required:\n"
                    + "  - LEI (Fund level access), or\n"
                    + "  - OeNB-ID (Fund level access), or\n"
                    + "  - Segment ISIN, or\n"
                    + "  - ShareClass ISIN\n");
        }
        ...
        for (String oenbId : oenbIdList) {
            if (oenbId.length() < 2 || oenbId.length() > 8) {
                errorMessage.append("• Invalid OeNB-ID format: '").append(oenbId).append("' (must be 2-8 digits)\n");
            }
        ...
        // XSD line 460-469: ISIN must be exactly 12 characters (2 letters + 9
