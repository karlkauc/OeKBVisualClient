...
/**
 * Controller for Own Data Downloaded page - shows download statistics (who
 * downloaded my data)
 */
public class OwnDataDownloaded implements Initializable {
    ...
    private void setupComboBoxes() {
        ...
        fdpContentCombo.setItems(FXCollections.observableArrayList("FUND", "DOC", "REG"));
    ...
    private void setupTable() {
        ...
        downloadedByCol.setCellValueFactory(
                cellData -> new SimpleStringProperty(cellData.getValue().getDataSuppliersString()));
        downloadedByCol.setCellFactory(column -> new TableCell<>() {
            @Override
            protected void updateItem(String item, boolean empty) {
                super.updateItem(item, empty);
        ...
        accessCol.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getAccessInfo()));
        ...
        downloadedInfoTable.getColumns().addAll(typeCol, contentDateCol, downloadDateCol, downloadedByCol, accessCol,
    ...
    private void loadStatistics() {
        ...
        new Thread(() -> {
            try {
                ...
                List<DownloadedInformationEntry> entries = downloadedInformationDao
                        .getDownloadedInformationEntries(dateFrom, dateTo, fdpContent, params);
    ...
    private void filterEntries() {
        ...
        List<DownloadedInformationEntry> filtered = allEntries.stream().filter(entry -> {
            // Filter by search text
            if (!searchText.isEmpty()) {
                String entryText = String.format("%s %s %s %s %s %s %s %s", entry.getContentType(),
                        entry.getFormattedContentDate(), entry.getFormattedDownloadDateTime(),
                        entry.getDataSuppliersString(), entry.getAccessInfo(), entry.getIdentifierSummary(),
                        entry.getDetails(), entry.getProfilesString()).toLowerCase(java.util.Locale.ROOT);
